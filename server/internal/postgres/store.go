...
func (s *Store) Ping(ctx context.Context) error {
	...
	_, err := s.pool.Exec(ctx, `SELECT i.id,d.id,c.id,s.token_hash,ca.media_generation,md.candidates FROM identities i
        ...
        JOIN contacts co ON false JOIN calls ca ON false JOIN call_invites ci ON false JOIN media_descriptions md ON false LIMIT 0`)
	return err
}
...
func (s *Store) Migrate(ctx context.Context) error {
	...
	if !applied {
		if _, err = tx.Exec(ctx, callsSchema); err != nil {
			return err
		}
	...
	if !applied {
		if _, err = tx.Exec(ctx, contactsSchema); err != nil {
			return err
		}
...
func (s *Store) Cleanup(ctx context.Context, now time.Time) error {
	...
	if _, err := s.pool.Exec(ctx, `DELETE FROM calls WHERE expires_at < $1::timestamptz - interval '24 hours'`, now); err != nil {
		return err
	}
