...
Go 控制面：已实现 Phase B 服务端认证、PostgreSQL 持久化、邀请兑换、通话状态与认证 WebSocket 查询。Android 已实现开发认证客户端，尚未接入通话页面；SDP/ICE、TURN 尚未接入，尚未部署 Cloud Run。
...
通话控制增量：共 14 项服务端测试覆盖身份和通话；验证真实 PostgreSQL、Linux race、vet、build 与容器构建。协议和版本 2 迁移见通话协议。
