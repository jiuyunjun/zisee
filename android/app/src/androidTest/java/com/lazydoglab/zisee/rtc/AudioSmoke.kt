...
import com.lazydoglab.zisee.rtc.audio.processing.DeepFilterNetEngine
    ...
    private fun processingFallback(context: Context, output: Bundle) {
        ...
        try {
            for (reason in listOf("thermal", "memory", "format")) {
                var fallbacks = 0
                val processor = com.lazydoglab.zisee.rtc.audio.processing.AudioProcessingEngine(context) { fallbacks++ }
                try {
                    processor.prepare()
                    check(processor.configure(track, com.lazydoglab.zisee.rtc.audio.processing.NoiseSuppressionMode.AUTO))
                    if (processor.stats().state != com.lazydoglab.zisee.rtc.audio.processing.AiState.ACTIVE) {
                    ...
                    check(processor.stats().state == com.lazydoglab.zisee.rtc.audio.processing.AiState.DEGRADED)
