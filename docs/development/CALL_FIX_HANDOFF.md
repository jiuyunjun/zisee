...
- 2026-09-11 01:48 AR 点击根因（已修，模拟器验证，待真机）：A 现场 uiautomator dump 有 TextureView 与工具条，但无“AR 现场，可轻点放置所选标记”点击层。真机时序是画面先上屏、数秒后才 TRACKING，onArTap 由 null 变为非 null；VideoRenderer 的 BoxWithConstraints 子组合保留首次捕获的 null（与此前圆角 cornerPx 同一机制），点击层永不出现。新增预览场景 ar-late-tracking（先 SCANNING，1s 后 TRACKING），CallArTapSmoke 两场景都跑：修复前 ar-late-tracking 失败“Marker layer never composed”。第一版修复（在 BoxWithConstraints 外用 rememberUpdatedState 包 onArTap、子组合内读 .value）无效：临时日志证实主画面 VideoRenderer 已收到 tap=true，但子组合仍不组合点击层，原因未查明。最终修复：BoxWithConstraints 只保留 AndroidView；点击层移到外层 Box（普通组合，直接判断 onArTap），画面尺寸由外层 onSizeChanged + 视频宽高比计算，仍只覆盖画面、黑边点击照旧切菜单。模拟器 arTap 两场景 PASS（shown + frame=none），thumbnailSwaps 回归 PASS，单测通过。真机仍需确认点击后为 placed 而非 frame=none（真机帧应带 AR 身份）。
- 2026-09-11 02:01 真机确认 AR 点击成功（AR_TAP placed ×14）。
- 2026-09-11 AR 画面两侧横向拖影（已改待真机）：ArCoreBackend 未选相机配置；视频按 CPU 图像视野（IMAGE_NORMALIZED→TEXTURE_NORMALIZED）采样 GPU 纹理，默认配置若 CPU 图像 4:3、纹理 16:9，角点落在纹理外，CLAMP_TO_EDGE 重复边缘列形成两侧拖影（推断，待 AR_CAMERA_CONFIG 日志确认 A 的尺寸）。改为创建会话后选后置 30fps、图像与纹理同宽高比、纹理最大的配置，无匹配则保留默认；新增 AR_CAMERA_CONFIG image/texture/matched/configs 日志。若 matched=false 仍拖影，需改为按纹理有效区裁剪并同步标记坐标换算。
- 2026-09-11 02:25 真机确认拖影消失：A 的 AR_CAMERA_CONFIG image=1280x720 texture=1920x1080 matched=true configs=3（此前默认配置宽高比不一致，推断成立）。
- 2026-09-11 放置判定放宽（单测覆盖，待真机体验）：用户反馈不是表面就放不了。SpatialResolver 原先深度只取点击处单个 160x120 深度格、为 0 即放弃；平面要求命中点严格在已识别多边形内。改为：深度中心格有效照旧，否则取周围 5x5 有效格中位数；平面先找多边形内命中，没有再接受边缘外 0.25m 内命中。仍拒绝离边缘 0.4m 等远处点击。未做：无任何表面时沿射线按估计距离放“悬空”标记（精度与走动偏差是产品取舍，待用户决定）。
