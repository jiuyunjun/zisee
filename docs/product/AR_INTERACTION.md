...
M3 跨模式目标以 [M3 通话多任务专项](CALL_MULTITASKING.md) 为准：后台不再结束基础通话；现场方离开现场全屏结束 AR、清空空间标记且不自动恢复相机，指导方缩窗仅观看。该策略尚待 M3 实现，当前代码仍是后台结束通话。
...
| 我打开后，对方是什么样子？ | 我方后摄现场成为协作画面；对方自动加入标记。对方摄像头保持原有开关状态，不因我开启 AR 而自动开启。 |
| 对方也要打开 AR 吗？ | 不需要。对方会自动成为指导方，无需运行 ARCore；不支持 AR 的手机也可以指导，但必须支持本应用的协作协议与视频显示。 |
...
- **加入标记**：对已开启的对方现场进行协作，成为指导方。默认自动完成；退出或被现场方撤权后保留为手动重试入口。只增加交互工具，不接管本机摄像头。
...
| 就绪 | “我的现场 · 可放置标记”；显示对方是否已加入 | 自动加入对方现场；握手完成后显示标记工具，仍可主动退出为仅观看 |
| 协作 | 看到本地实时场景与双方已确认的标记 | 看到同一现场视频与确认后的标记；其视角是 A 的摄像头视角，不是自由三维漫游 |

安装/权限返回后必须再点击开启；不能因迟到回调、旋转重建或重新前台而自动发送现场画面。当前应用进入后台会结束通话；M3 落地后改为现场方离开结束 AR、语音继续，指导方仅暂停交互。授权/安装外部页面与 Home 分开识别，细则见 M3 专项 §4。
...
自动加入后，指导方可选择“退出标记”进入仅观看，仍看到现场和已存在的标记，工具栏不出现；画面保留“加入标记”轻入口。退出或被现场方撤权后，同一现场的重复就绪消息不会触发自动重入。用户手动再加入后，只有握手和标记状态同步完成才启用工具，不能把按钮按下当作已加入成功。
...
“我能标记”与“我能开相机”分开授权。现场方在参与菜单中可关闭“允许对方标记”，对方立即回到仅观看，停止接受新的修改；已有标记保留。重新允许不会让已退出或已被撤权的对方反复自动加入；指导方可手动重试。首版不提供任意编辑对方文字、拖动对方锚点或接管手机。
...
| 后台、锁屏、跳转安装页 | M3 目标：离开现场结束 AR，不自动恢复相机；音频继续 | 缩窗仅观看，锁屏暂停交互；不结束对方现场 | 外部授权页面独立建模；现场失效须重新开启，不恢复旧标记 |
...
| 挂断、后台、退出启动中、迟到授权 | 清理相机/EGL/锚点/排队命令，无自动复活；M3 目标按角色处理后台且保留基础通话 |
...
- 对齐 M3 多任务目标：现场方离开结束现场保语音、指导方缩窗仅观看；明确当前代码仍未实现后台通话。
