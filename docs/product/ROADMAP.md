...
M3  通话多任务、屏幕共享与 2D 标注    进行中（M3-F 屏幕共享框架已落地；服务、UI、RTC 屏幕传输待接入）
...
```text
M3-F 框架已完成；下一步 M3-A 后台通话基础 → M3-B 小窗 → M3-C 共享接入 → M3.1
```
...
- 当前里程碑切换为 M3 开发，先交付 M3-F 共享状态机和 Android 投影框架。
- 区分已实现框架与尚未接入的服务/授权/UI/RTC 屏幕流，保留 M3-A/B/C 接入顺序和 AR 真机门槛。
