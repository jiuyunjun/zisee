...
M3  通话多任务、屏幕共享与 2D 标注    专项设计已建立（Draft；应用实现未开始，Annotation2D 仅模型占位）
...
离开通话页面仍能持续交流；打开其他 App 时能看对方，双方清楚知道正在分享什么。

专项设计：[CALL_MULTITASKING.md](CALL_MULTITASKING.md)（Draft，设计不等于已经实现）。

## 范围与交付顺序

1. M3-A：活动通话 owner、前台服务、通话通知，解除 Activity 停止即挂断的耦合。
...
- Home 不挂断；PiP 关闭/不可用、锁屏、返回前台和任务移除有明确且可验证的行为。
- 屏幕共享在双设备间完成授权、首帧、后台操作、系统停止的完整闭环。
- Show Me / 共享 / AR 组合与双方状态符合专项矩阵，不能自动重开用户已关闭的相机或过期投影。
- 没有 Camera / MediaProjection / EGL / 前台服务泄漏，语音优先。
- 完成专项 §9 对应真机矩阵；文档设计或模拟预览不能替代设备验收。

---

# 8. M3.1：2D 远程标注

以 [CALL_MULTITASKING.md](CALL_MULTITASKING.md) §6 为准：先做同画面实时 Pointer，再做共享/相机的双方定格讲解、圈/箭头/画笔与撤销清除。共享方在其他 App 时通过通知返回讲解，不把仅观看方能见到的笔迹算完成；跨 App 覆盖指示层属于后续可选增强。
...
```text
先完成 M3 多任务与协作专项设计，再按 M3-A → M3-B → M3-C → M3.1 小步实现
```
...
按本次任务方向先做 M3 专项设计，覆盖后台通话、三类小窗、Show Me、屏幕共享、2D 标注和 AR 的组合。方案见 [CALL_MULTITASKING.md](CALL_MULTITASKING.md)，本次不实施应用代码。

AR 基础闭环仍未经过双设备端到端验证，以下清单保留为 M4 退出与 M3 AR 集成验收门槛；不把它作为完成本轮设计的前置阻塞。现有实现的回环失败问题也仍待判定。
...
- M3 按专项 §9 逐步实现、验证和提交，先保证后台通话再做共享。
- AR 真机通过后，其一致性与权限完善继续按 AR_INTERACTION.md §12 排期，不自动抢占 M3。
- AR 若证伪或代价过高，降级其产品入口并保留框架，不影响普通通话、共享与 2D 闭环；M3 AR 组合项明确标未通过，不能宣称全量完成。
...
- M3 扩展为后台通话、应用内迷你通话、系统 PiP、屏幕共享与 2D 协作；建立专项设计及分步退出条件。
- 当前优先事项调整为先完成 M3 设计，保留 AR 真机门槛，未声称应用实现完成。
