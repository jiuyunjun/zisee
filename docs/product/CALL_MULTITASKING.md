...
本轮交付专项设计，不修改应用代码。下文是目标行为，**不是当前 APK 能力声明**。M3 从“增加屏幕 Track”扩展为“离开通话页面也能持续交流，并清楚知道双方正在分享、观看和标记什么”。AR 空间交互继续由 [AR_INTERACTION.md](AR_INTERACTION.md) 定义；M3 落地后，跨模式生命周期以本文为准。
...
| A 共享 → Home | 看 B 人像/当前普通视频，没有视频则头像 | 共享及音频继续；已明确保留的单摄按能力继续 | B 看 A 屏幕，可请求讲解 |
...
2. 相机默认选择“仅屏幕 + 语音”；若已有前摄开启，用户可明确选择“保留人像”。不因共享从关相机自动开前摄。Show Me 明示双摄将暂停；首版不保留后摄 + 屏幕或三路同时发送。
...
7. 停止后仅在全屏、权限仍有效且用户未更改相机意图时恢复进入前相机；后台不自动重开双摄。换共享目标或系统已停止后重新共享，必须再次授权。
...
| 当前能力 | 可同时使用 | 不兼容动作与转换 |
| --- | --- | --- |
| 普通人像/单后摄 | 音频、普通视频、对方双摄、2D 定格、PiP | 开屏幕时按 §5.1 选择；后摄不随共享保留 |
| Show Me | 独立前后摄、对方普通/双摄、2D 定格 | 本机共享暂停双摄，保留人像需显式选择；PiP 本机仅后摄 |
| 一方共享屏幕 | 双向音频、共享方可选前摄、观看方普通相机 | 观看方首版最多单摄，Show Me 暂停辅摄并告知；负载紧张再降摄像头 |
...
| 共享中点关相机 | 只关摄像头，屏幕继续；按钮文案明确“关闭摄像头”，停止共享独立 |
...
| 步骤 | 独立交付 | 退出条件 |
| --- | --- | --- |
| M3-D（本次） | 专项设计、关联文档与路线图 | 组合规则、两端反馈、生命周期及验收可评审；未实现项明确 |
| M3-A | 活动通话 owner、前台服务、通知、生命周期 | Home 不挂断；全屏/系统界面/锁屏/任务移除行为正确；挂断资源归零 |
...
以下矩阵是未来实现验收要求，本次未执行 APK 构建或真机测试：
